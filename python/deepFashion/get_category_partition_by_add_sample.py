...
#python python/deepFashion/get_category_partition_by_add_sample.py -i data/deepFashion/Anno/list_category_img.txt -d data/deepFashion
...
def get_label_partition_num(label_num_dict):
    ...
    train_num_mean = int(np.mean(train_num))
    #cond_train_num = train_num[sample_num > 10000]
    #train_num_mean = int(np.mean(cond_train_num))

    return label_partition_num, train_num_mean

def get_partition_datasets(label_num_dict, input_txt, output_dir):
    label_partition_num, train_num_mean = get_label_partition_num(label_num_dict)
    ...
    fo_train = open(output_dir+'/category_add18_addtrain_label.txt', 'w')
    ...
    for label in labels:
        ...
        train_lines = list(array(label_partition_dict[label])[:train_num])
        if label=='19':
            times = 18435/train_num
            train_lines = times*train_lines + random.sample(train_lines, 18435 - times*train_num)
        else:
